defmodule Oasis.Parser do
  ...
  @spec extract_metadata_from_operations(swagger_data :: Swagger.t()) :: %{String.t() => Metadata.t()}
  def extract_metadata_from_operations(%Swagger{paths: paths} = swagger_data) when is_struct(swagger_data) do
    paths
    |> Enum.map(fn path ->
      Path.new()
    end)
